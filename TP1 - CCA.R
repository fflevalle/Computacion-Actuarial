...
summary(choco)
choco$Cocoa_Percent <- as.numeric(sub("%", "", choco$Cocoa_Percent))/100

str(choco)
factor_vars <- c("Company","Bean_Specific_Origin","Company_Location","Bean_type","Bean_Broad_Origin","Review_Date")
choco[factor_vars] <- lapply(choco[factor_vars], function(x) factor(x, exclude = NULL))

#NA

#No tomaria en cuenta esas variables dado que la cantidad de valores NA es muy grande y cualquier modificacion podria alterar 
...
#Para las variables bean type y bean broad origin, dado que la cantidad de NA aqui es muy grande se creara una categoria nueva en donde
#iran aquellos valores que sean NA

choco

correlation <- data.frame(choco$REF, as.numeric(choco$Review_Date), choco$Cocoa_Percent, choco$Rating)
...
dummy <- choco %>%
  ...
  dummy$Rating <- NULL
dummy$bin_Rating <- factor(dummy$bin_Rating)  
...
form <- formula(bin_Rating ~ Company + Bean_Specific_Origin + Review_Date + Company_Location + Bean_type + Bean_Broad_Origin)
form <- formula(bin_Rating ~ Review_Date + Company + Company_Location + Bean_Broad_Origin)

dummycols <- model.matrix( form  , data = dummy)
...
linearmodel <- lm(data = train_choco, Rating ~.)
summary(linearmodel)

write.csv(t(glance(linearmodel)), file = "glancelm.csv")
write.csv(broom::tidy(linearmodel), file = "tidy1.csv")

linearmodel2 <- lm(data = train_choco, Rating ~ Cocoa_Percent + Bean_sp_OriginG + CompanyG + Company_locG + Bean_br_OriginG)
...
#Probemos como se desempeña el modelo logistico
# Modelo de prueba
pred.prob <- predict(logisticmodel, test_dummy, type="response") 
pred.Rating <- ifelse(pred.prob >= 0.490, 1 , 0)
str(pred.prob)
